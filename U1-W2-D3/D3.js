...
for(let i2=0; i2 < starWarsCharacters.length; i2++){
  if(starWarsCharacters[i2].gender === "female"){
    femaleCharacters.push(starWarsCharacters[i2])
...
switch(starWarsCharacters.eye_color){
  ...
  default:
    eyeColor.blue_gray = [starWarsCharacters]
}
...
const imax = starWarsCharacters.length

let i3= 0

while(i3 < imax){

  crewMass += starWarsCharacters[i3].mass
  i3++
...
if(crewMass < 500){
  risposta = "Ship is under loaded"
}else if (crewMass >= 500 && crewMass < 700){
  risposta = "Ship is half loaded"
}else if (crewMass > 700 && crewMass < 900){
  risposta = "Warning: Load is over 700"
}else if (crewMass > 900 && crewMass < 1000){
  risposta = "Critical Load: Over 900"
}else{
  risposta = "DANGER! OVERLOAD ALERT: escape from ship now!"
}
...
for(let i4=0; i4 < starWarsCharacters.length; i4++){

let risp = prompt(`Vuoi modificare il genere del personaggio ${starWarsCharacters[i4].name}? Rispondi si/no`)

if(risp.toLowerCase === "si" && starWarsCharacters[i4].gender === "n/a"){
  starWarsCharacters[i4].gender = "robot"
}
}
